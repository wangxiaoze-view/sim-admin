...
export function useFlow() {
  ...
  const { getTheme, isDark } = useSwitchDark()

  const lf = ref<LogicFlow>()

  const getDark = computed(() => {
    return getTheme.value.mode === 'dark'
  })
  ...
  const init = () => {
    lf.value = new LogicFlow({
      ...
      background: {
        backgroundColor: getDark.value ? '#141414' : '#ffffff',
      },
    })
    initMenu()
    initPattern()
